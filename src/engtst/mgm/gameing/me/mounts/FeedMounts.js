...
export default class FeedMounts extends BaseClass{
	 ...
	 constructor( ani)
	{
		...
		this.btn_feed=new XButtonEx2(GmPlay.xani_nui2);
	...
	Draw()
	{
		...
		M3DFast.gi().DrawTextEx(this.iX+30, this.iY+30, "选择丹药或烹饪", 0xff003e57, 30, 101, 1, 1, 0, 0, 0);
...
//			if(this.lockgoods.iTid==154 || this.lockgoods.iTid==155 || this.lockgoods.iTid==156)
			{
				if(this.btn_feed.ProcTouch(msg, x, y))
				{
					...
					{
						GmProtocol.gi().s_FeedMounts(FeedMounts.pms.iMid, this.lockgoods.iGid,this.lockgoods.iTid);
						XStat.gi().PopStat(1);
					}
					return true;
				}
...
{
	XStat.gi().PushStat(XStat.GS_FEEDMOUNTS);
	FeedMounts.pms=p;
}