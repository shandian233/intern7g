...
export default class SmallSpeaker extends BaseClass{
	...
	Draw()
	{
		...
		M3DFast.gi().FillRect_2D(this.iX+20, this.iY+20, this.iX+this.iW-180,this.iY+this.iH-20,0xff000001);
	...
	ProcTouch( msg, x, y)
	{
		...
		if(this.btn_send.ProcTouch(msg, x, y))
		{
			...
			{//发送小喇叭内容
				...
				{
					GmProtocol.gi().s_UseGoods(SmallSpeaker.goods.iGid, 1,this.in_speak.sDetail);
					XStat.gi().PopStat(1);
				}
			}
		}
...
SmallSpeaker.Open=function( g)
{
	...
	XStat.gi().PushStat(XStat.GS_SMALLSPEAKER);
