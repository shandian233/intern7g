...
//注入资金
export default class FTGetOut extends BaseClass{
		...
		if(this.btn_ok.ProcTouch(msg, x, y))
		{
			if(this.btn_ok.bCheck())
			{
				if(this.in_money.sDetail.length>0)
				{
					...
					{//确定取出
						GmProtocol.gi().s_FTOperate(2,i,0);
					}
					XStat.gi().PopStat(1);
				}
			}
...
{
    var ftc;
    if(XStat.gi().iXStat==XStat.GS_FTGETOUT)
    {
        ftc= XStat.gi().oCurrentView;
    }
    else
    {
        ftc= XStat.gi().PushStat(XStat.GS_FTGETOUT);
    }
