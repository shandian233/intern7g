...
export default class Gov_SetHostility extends BaseClass{
	 ...
	 constructor( a)
	{
		...
		this.in_hid=new XInput(GmPlay.xani_ui);
		this.in_hid.sDetail="";
...
//		M3DFast.gi().DrawTextEx(this.iX+this.iW/2, this.iY+20+15, "选择所需建造的建筑", 0xff005b41, 30, 101, 1, 1, 0, -2, -2);
		...
		M3DFast.gi().DrawTextEx(this.iX+this.iW/2,this.iY+30+22, "设置敌对", 0xff000000, 25, 101, 1, 1, 0, -2, -2);
	 ...
	 ProcTouch( msg, x, y)
	{
		...
		if(this.btn_ok.ProcTouch(msg, x, y))
		{
			if(this.btn_ok.bCheck())
			{
				...
				{
					GmProtocol.gi().s_NewGovOperate(18, 5, 0, 0, 0, this.in_hid.sDetail);
...
{
    XStat.gi().PushStat(XStat.GS_GOV_SETHOSTILITY);
}