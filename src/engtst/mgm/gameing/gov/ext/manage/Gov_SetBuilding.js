...
export default class Gov_SetBuilding extends BaseClass{
	 ...
	 constructor( a)
	{
		...
		this.btn_building=new XButtonEx2(9);
		for(i=0;i<9;i++)
		{
			this.btn_building[i]=new XButtonEx2(GmPlay.xani_button);
	...
	ProcTouch( msg, x, y)
	{
		...
		for(i=0;i<9;i++)
		{
			if(this.btn_building[i].ProcTouch(msg, x, y))
			{
				...
				{
					GmProtocol.gi().s_NewGovOperate(18, 1, i, 0, 0, "");
...
{
    XStat.gi().PushStat(XStat.GS_GOV_SETBUILDING);
}