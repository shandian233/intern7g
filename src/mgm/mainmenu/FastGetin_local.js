...
export default class FastGetin_local  extends BaseClass{
	...
	constructor( ani)
	{
		...
		{
			this.btn_history[i]=new XButton(ani);
			this.btn_history[i].InitButton("主菜单按钮2");
			
			this.btn_delete[i]=new XButton(ani);
			this.btn_delete[i].sName="x";
			this.btn_delete[i].InitButton("中按钮1");
		}
		this.InitUserBtn();
		
		this.btn_autogetin=new XButton(ani);
...
//		j++;
		...
		if(j==0)this.btn_login.Move(GmConfig.SCRW/2-k/2, GmConfig.SCRH/2+50, k, 50);
		else this.btn_login.Move(GmConfig.SCRW/2-k/2, GmConfig.SCRH/2+100, k, 50);
		this.btn_login.Draw();
